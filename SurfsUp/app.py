...
# Query the dates and temperature observations from api
...
def tobs():
 ...
 # Calculate `TMIN`, `TAVG`, and `TMAX` for dates
...
def start_end(start, end):
    ...
    if valid_entry_start and valid_entry_end:
        ...
        tmin = results[0][0]
        tavg = f'{results[0][1]:.4f}'
        tmax = results[0][2]

        result_printout = [
            'Entered Start Date: ' + start,
    ...
    if not valid_entry_start and not valid_entry_end:
        return jsonify({
            "error": f"Input Start {start} and End Date {end} not valid. Date Range is {date_range_min_str} to {date_range_max_str}"
        }), 404

    if not valid_entry_start:
        return jsonify({
            "error": f"Input Start Date {start} not valid. Date Range is {date_range_min_str} to {date_range_max_str}"
        }), 404

    if not valid_entry_end:
        return jsonify({
            "error": f"Input End Date {end} not valid. Date Range is {date_range_min_str} to {date_range_max_str}"
        }), 404